...
are new; no schema, store method or env var moved.
...
- [x] **v0.75.0** through the test-gated pipeline. `.github/` untouched
  ...
  **public** (anonymous pull 200 on both tags,
  ...
  attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.65.0 -> **0.66.0**, a minor alongside an app minor
- [x] Routes **209 -> 211**; `store.Store` unchanged at **229**; migration
  high-water unchanged at **`0059`**
- [x] Both READMEs restated; every `Reflects:` header, both READMEs' *What
  works today* range, `docs/README.md`, `NIS2-COMPLIANCE.md`'s evidence row
  and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now do** — share a
  desktop, join one, and keep it a view rather than a data channel — and
  states what has not changed: SSH sharing and the invite rules
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
- [x] Routes **209 → 211**; no schema, no store method (surface stays
  **229**), no env var. Audit: `session.share_join_denied` gains
  ...
  gains `via:portal`. **Released by Phase 261 as v0.75.0** — a minor, since
  routes moved
...
Releases **256** — level-tiered and direct-manager approval. A **minor**:
the schema (`0059`) and the store surface moved; no route or env var was
added.
...
- [x] **v0.73.0** through the test-gated pipeline. `.github/` untouched
  ...
  **public** (anonymous pull 200 on both tags,
  ...
  attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.63.0 -> **0.64.0**, a minor alongside an app minor
- [x] `store.Store` **228 -> 229**; migration high-water **`0058` ->
  `0059`**; routes unchanged at **207**
- [x] Both READMEs restated; every `Reflects:` header, both READMEs' *What
  works today* range, `docs/README.md`, `NIS2-COMPLIANCE.md`'s evidence row
  and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now do** — say who
  must approve and in what order, name a direct manager, see the chain
  where approvals happen — and states what has not changed: every target
  and safe without a chain keeps the N-of-M count it had
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **254** — the in-portal SSH terminal. A **minor**: three routes are
new; no schema, env var or store method moved.
...
- [x] **v0.72.0** through the test-gated pipeline. `.github/` untouched
  ...
  **public** (anonymous pull 200 on both tags,
  ...
  attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.62.0 -> **0.63.0**, a minor alongside an app minor
- [x] `store.Store` unchanged at **228**; migration high-water unchanged at
  **`0058`**; routes **204 -> 207**
- [x] Both READMEs restated; every `Reflects:` header, both READMEs' *What
  works today* range, `docs/README.md`, `NIS2-COMPLIANCE.md`'s evidence row
  and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now do** — open an
  SSH target in the browser, trust the door — and states what has not
  changed: the terminal decides no access of its own
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **252** — credential-level grants. A **minor**: the schema (`0058`)
moved; no route, env var or store method was added.

- [x] **v0.71.0** through the test-gated pipeline. `.github/` untouched
  since v0.58.1 and nothing the workflow *reads* has changed either, so no
  rehearsal. Published 2026-09-16 as `ghcr.io/morandeirachema/pamv1:0.71.0`
  (also `latest`), digest
  ...
  **public** (anonymous pull 200 on both tags,
  ...
  attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.61.0 -> **0.62.0**, a minor alongside an app minor
- [x] `store.Store` unchanged at **228**; migration high-water **`0057` ->
  `0058`**; routes unchanged at **204**
- [x] Both READMEs restated; every `Reflects:` header, both READMEs' *What
  works today* range, `docs/README.md`, `NIS2-COMPLIANCE.md`'s evidence row
  and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now do** — grant one
  credential rather than the whole target, trust the others stay closed, see
  the scope where access is reviewed — and states what has not changed:
  every pre-existing grant covers its whole target exactly as before
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **240** — session lifetime, grant expiry and time frames. A
**minor**: the schema (`0053`), the store surface, two env vars, two audit
actions and the grants/members request bodies all moved.

- [x] **v0.66.0** through the test-gated pipeline. `.github/` untouched
  since v0.58.1 and nothing the workflow *reads* has changed either, so no
  rehearsal. Published 2026-09-03 as `ghcr.io/morandeirachema/pamv1:0.66.0`
  (also `latest`), digest
  ...
  **public** (anonymous pull 200 on both tags, both resolving to the same
  ...
  `pam-agent` binaries, the SPDX SBOM and `SHA256SUMS` attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.56.1 -> **0.57.0**, a minor alongside an app minor
- [x] `store.Store` **222 -> 223**; migration high-water **`0052` ->
  `0053`**; routes unchanged at **195**
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now do** — bound
  every session's length and idleness, put an end date or a weekly window
  on a standing grant — and states what has not changed: every existing
  grant is unbounded, both knobs default off
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
- [x] Schema (`0053`), store surface (223), two env vars, two audit actions,
  ...
  **195**, their bodies extended. **Released by Phase 241 as v0.66.0** — a
  minor, since the schema moved
...
schema or route moved beyond the two new ones, but a new env var pair and
observable behaviour exist when they are set — an approver can decide a
request from Slack — so it is not a patch.
...
- [x] `store.Store` unchanged at **220**; migration high-water unchanged at
  `0051`; routes **193 -> 195** (the two new Slack routes)
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now configure** —
  Slack chat-ops approval — then states plainly what has not changed: no
  schema, nothing forced on by default
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **232** — on-call/schedule-aware access gating. A **minor**: no
schema or route moved, but a new env var (`PAM_ONCALL_ATTEST_URL`) and
observable behaviour exist when it's set — a deployment can now refuse a
connect or an authenticated call to someone who isn't on call — so it is
not a patch.

- [x] **v0.63.0** through the test-gated pipeline. `.github/` untouched
  since v0.58.1 and nothing the workflow *reads* has changed either, so no
  rehearsal. Published 2026-08-31 as `ghcr.io/morandeirachema/pamv1:0.63.0`
  (also `latest`), digest
  ...
  **public** (anonymous pull 200 on both tags, both resolving to the same
  ...
  `pam-agent` binaries, the SPDX SBOM and `SHA256SUMS` attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm
  chart `version` 0.53.1 -> **0.54.0**, a minor alongside an app minor
- [x] `store.Store` unchanged at **220**; migration high-water unchanged at
  `0051`; routes unchanged at **193**
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **what an operator can now configure** —
  on-call gating — then states plainly what has not changed: no schema, no
  route, nothing forced on by default
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
**patch**: no schema, route or env var moved, and nothing reachable through
a handler changed — `store.EffectiveMFAFactors` had no production caller,
and the SFTP-mode enum now parses through the same function either way.

- [x] **v0.62.1** through the test-gated pipeline. `.github/` untouched
...
- [x] `CHANGELOG.md` leads with the fact that **nothing an operator does
  changes** — the fixes are all internal (doc rows, dead code, a
  duplicate-validation cleanup, and this project's own bookkeeping)
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **226** — the MCP revision negotiated, not pinned. A **minor**: no
schema, env var, route or audit action moved, but every MCP client sees a
different `initialize` answer, batches are accepted and a header is checked,
so the wire behaviour is visible and it is not a patch.

- [x] **v0.62.0** through the test-gated pipeline. `.github/` untouched since
...
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **what an MCP client sees now**, then states
  plainly what has not changed — the transport — so nobody reads "2025-06-18"
  as "Streamable HTTP"
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **224** — the trust bundle follows the file. A **minor**: no schema,
env var, route or audit action moved, but the behaviour is observable and was
an operational step until now (a SPIRE key rotation meant restarting PAMv1),
so it is not a patch in the v0.18.2/v0.54.1/v0.58.2 sense.

- [x] **v0.61.0** through the test-gated pipeline. `.github/` untouched since
  v0.58.1 and — the rule Phase 214 added — nothing the workflow *reads* has
  changed either (same repository name, same runner image), so no rehearsal.
  Published 2026-08-27 as `ghcr.io/morandeirachema/pamv1:0.61.0` (also
  `latest`), digest
  ...
  **public** (anonymous pull 200 on both tags, both resolving to the same
  ...
  `pam-agent` binaries, the SPDX SBOM and `SHA256SUMS` attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm chart
  `version` 0.51.0 -> **0.52.0**, a minor alongside an app minor
- [x] `store.Store` unchanged at **220**; migration high-water unchanged at
  `0051`; routes unchanged at **193**
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **the restart that is no longer needed** and
  states what a broken rewrite of the bundle does (keeps the last good keys,
  logs) so an operator is not surprised in either direction
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
...
Releases **222** (and 221's resync). A **minor**, because the schema moves
(migration `0051`, `broker_tokens.subject`) — the precedent of every
migration-carrying release — and here too nothing needs doing: it applies on
startup, and a token minted before it keeps spending for its remaining
minutes. No env var, route or audit action moved. What changed is that a
resume token is now worth nothing to any agent but the one that parked the
call, which closes the 2026-08-26 audit in full.

- [x] **v0.60.0** through the test-gated pipeline. `.github/` untouched since
  v0.58.1 and — the rule Phase 214 added — nothing the workflow *reads* has
  changed either (same repository name, same runner image), so no rehearsal.
  Published 2026-08-27 as `ghcr.io/morandeirachema/pamv1:0.60.0` (also
  `latest`), digest
  ...
  **public** (anonymous pull 200 on both tags, both resolving to the same
  ...
  `pam-agent` binaries, the SPDX SBOM and `SHA256SUMS` attached
- [x] All pins via the sweep — exactly one release under `deploy/`. Helm chart
  `version` 0.50.0 -> **0.51.0**, a minor alongside an app minor
- [x] `store.Store` unchanged at **220** (two signatures changed); migration
  high-water `0050` -> **`0051`**; routes unchanged at **193**
- [x] Both READMEs restated; every `Reflects:` header, `docs/README.md`,
  `NIS2-COMPLIANCE.md`'s evidence row and this banner
- [x] `CHANGELOG.md` leads with **what a leaked token is now worth** — nothing
  — and states that nothing configured changes, the migration needs no action,
  and the pre-upgrade tokens' one-window grace
- [x] The tag is pushed only **after** the release PR is confirmed merged
- [x] Full CI-gate sweep re-verified clean on `main` before tagging
