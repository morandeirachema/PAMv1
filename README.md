...
runs 0–227 and 229–276, and **every phase has shipped**, and the current
...
Phases 0–227 and 229–265, grouped by area. Every capability is exercised by tests and deploys as code.
...
- **Safes (delegated-access containers)** — group targets into a named **safe** with its own members; a member may connect to **every target in the safe** (an authorization path alongside per-target grants), and a `can_manage` member is a **delegated safe administrator**. Placing a target in a safe restricts it to the safe's members. `POST /api/safes`, `/api/safes/{id}/members`, `PUT /api/targets/{id}/safe`. A grant or membership may carry an **expiry** and a **time frame** (`"Mon-Fri 08:00-18:00 Europe/Madrid"`, Phase 240): outside them it does not admit, and a session admitted under one ends at its edge. `PAM_SESSION_MAX_MIN` / `PAM_SESSION_IDLE_MIN` bound every session's length and idleness. A membership names what it confers (Phase 246): `use` (connect), `retrieve` (reveal / check out) and `approve` (decide the safe's access requests without the global approver role).
...
broker four-eyes sound — released steadily as **v0.10.0 → v0.0.78** (the numbering moved to 0.0.x at v0.0.77). Releases
...
| ~~**Windows session probe**~~ **✅ shipped (Phase 266)** | Telemetry and blocking *inside* a brokered RDP session — which programs the operator runs and where they connect, with block rules for both (EDR-style sensors; the desktop counterpart of command control) | The same `pam-agent`, `PAM_AGENT_MODE=probe`, launched at log-on inside the operator's Windows session **with that user's own token**: it reports the session's processes and connections over one channel pam-server opens toward it and ends what the rules match — only what the user could end, no firewall rules, audited per enforcement, never a containment boundary. Menu 34; `/api/probes*`, `/api/probe-rules*` |
...
### Tier 8 — CyberArk PAM / WALLIX Bastion / Teleport RBAC-and-feature research (2026-09-03)

A third pass over the three vendors this doc has compared itself against
longest, this time with the **RBAC model and the feature set** as the
question rather than a single differentiator. Every candidate was
fact-checked against the routes, env vars, `store` types and capability list
before being listed. Rows are added as each phase ships.

| Gap | Leaders | PAMv1 today |
|---|---|---|
| ~~**Session max duration and idle timeout**~~ **✅ shipped (Phase 240)** | Teleport `max_session_ttl` / `client_idle_timeout`, CyberArk PSM session time-out, WALLIX inactivity timeout | `PAM_SESSION_MAX_MIN` / `PAM_SESSION_IDLE_MIN`, one lifetime monitor over every session type; idle counts **operator input** only; each end audited `session.killed` with its reason |
| ~~**Expiry on standing grants and safe memberships**~~ **✅ shipped (Phase 240)** | CyberArk safe-membership expiration date, Teleport access-list member expiry | `expires_at` on a target grant or safe membership; an expired row does not admit, keeps the target gated, and is swept and audited (`grant.expired`, `safe.member.expired`); a session admitted under it ends **at** the expiry |
...
| ~~**Share a live RDP/VNC session** (view, or view and control)~~ **✅ shipped (Phase 260)** | Access Manager *Session invite* | Phase 116's four-eyes invite now shares a desktop: an internal invitee redeems it in the portal (main menu **33**), an external guest from the emailed link, and both join the owner's guacd connection — read-only, or with keyboard and mouse only; the clipboard and files are never shared, a kick or the session ending closes the join |
...
| ~~**Per-target and per-grant sub-protocol rights**~~ **✅ shipped (Phase 270)** | per service: RDP clipboard up/down/file, drive, printer, smartcard, audio in/out; SSH shell, exec, SCP up/down, SFTP, X11, forwards, agent | `rights` on a target and on a grant over `ssh_shell`/`ssh_exec`/`ssh_sftp`/`ssh_forward`/`ssh_x11`/`rdp_drive`/`rdp_printer`/`rdp_audio`/`rdp_audio_in`; grants narrow the target, deployment switches stay ceilings (`PAM_RDP_DRIVE`/`_PRINTER`/`_AUDIO`/`_AUDIO_IN`); refusals audited `session.right_denied`. Not covered: smartcard and agent forwarding (the proxy supports neither), clipboard file transfer (guacd has no separate switch) |
| ~~**Probe metadata as a session artifact**~~ **✅ shipped (Phase 271)** — *keystroke capture deliberately not* | the Session Probe emits process start/stop, foreground window title and masked keystrokes as searchable metadata beside the video; rules can `notify` as well as `kill` | a sealed, chained `.probe.log` per probe session with every process start (command line) and end, every foreground-window change and every rule match; listed as kind `probe`, searchable with the recordings; rules take `action: notify`. Keystrokes are not captured: a probe running as the user would be a keylogger the user can read, and the desktop recording already shows what was typed on screen |
| ~~**Trust-on-first-use host key and certificate store per target**~~ **✅ SSH shipped (Phase 272)** — *RDP not possible here* | the target's SSH key or RDP certificate kept on first contact, a check mode (fail on mismatch / accept first seen), a Certificates tab, "new fingerprint saved" and "wrong fingerprint" notifications | each SSH target's key pinned on first contact and checked after (`PAM_SSH_HOST_KEY_CHECK=tofu\|strict\|off`), a mismatch refused, audited with both fingerprints and alerted, reset by an administrator (Targets → 12, `DELETE /api/targets/{id}/host-key`); a known_hosts file stays authoritative. RDP cannot be pinned by PAMv1: guacd terminates the TLS and the server certificate never reaches it |
| ~~**Approval depth**~~ **✅ shipped (Phase 274)** — *"outside the time frame: require approval" open* | quorum as a number, the approver types a duration capped at the request, an approval timeout, comment/ticket mandatory per authorization, cancel an approved request mid-session, "outside the time frame: require approval instead of blocking" | approver comment (optional or `PAM_APPROVAL_COMMENT_REQUIRED`), `duration_min` capped at the request, `PAM_APPROVAL_TIMEOUT_MIN`, `POST /api/access-requests/{id}/cancel` ending the requester's sessions; quorum (N-of-M) and mandatory ticket were already there. Approval instead of a block outside a grant's time frame is a separate authorization semantic, still open |
...
**[v0.0.78](https://github.com/morandeirachema/pamv1/releases/tag/v0.0.78)**. The full list is in
...
> `ghcr.io/morandeirachema/pamv1:0.0.78`, is published and public.)
...
```bash
TAG=v0.0.78                       # the released version
...
#    is always lowercase, as an OCI reference must be.
cosign verify \
  --certificate-identity-regexp "(?i)^https://github.com/morandeirachema/pamv1/.github/workflows/release.yml@refs/tags/" \
...
```bash
kubectl -n pamv1 exec deploy/pamv1 -- /pam-server -version   # → pam-server 0.0.78 (<full commit sha>)
curl -s http://pamv1:8080/metrics | grep pam_build_info      # same, for monitoring
```

**Status:** **[v0.0.78](https://github.com/morandeirachema/pamv1/releases/tag/v0.0.78)
was released on 2026-09-22**, and every manifest here pins its tag, `0.0.78` — image
digest
`sha256:c0b896120a4df8faf0499a367883cb76af3d302140995aea9f300c2f8cfd1142`, public
...
`cosign verify` line for that exact image. (`v0.11.1` is a source tag only: its pipeline failed
