...
runs 0–127 and **every phase has shipped**, and the current
...
Phases 0–127, grouped by area. Every capability is exercised by tests and deploys as code.
...
broker four-eyes sound — released steadily as **v0.10.0 → v0.27.0**. Releases
...
**[v0.27.0](https://github.com/morandeirachema/pamv1/releases/tag/v0.27.0)**. The full list is in
...
> `ghcr.io/morandeirachema/pamv1:0.27.0`, is published and public.)
...
```bash
TAG=v0.27.0                       # the released version
...
```bash
kubectl -n pamv1 exec deploy/pamv1 -- /pam-server -version   # → pam-server 0.27.0 (<full commit sha>)
