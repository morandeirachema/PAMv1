...
runs 0–227 and 229–239, and **every phase has shipped**, and the current
tagged, cosign-signed release is
**[v0.65.1](https://github.com/morandeirachema/pamv1/releases/tag/v0.65.1)** (2026-09-03;
...
broker four-eyes sound — released steadily as **v0.10.0 → v0.65.1**. Releases
...
| **Per-session MFA and expiring user tokens** | Teleport `require_session_mfa`, CyberArk PSM re-authentication, WALLIX per-session MFA | MFA gates login and enrollment, not each connect; user API tokens never expire |
| **Admin lock of a human identity** (reason, optional expiry, sessions cut) | Teleport locks, CyberArk / WALLIX disable | only SCIM deactivation and delete; `cutUserAccess` exists, the flag and route do not |
...
**[v0.65.1](https://github.com/morandeirachema/pamv1/releases/tag/v0.65.1)**. The full list is in
...
> `ghcr.io/morandeirachema/pamv1:0.65.1`, is published and public.)
...
```bash
TAG=v0.65.1                       # the released version
...
```bash
kubectl -n pamv1 exec deploy/pamv1 -- /pam-server -version   # → pam-server 0.65.1 (<full commit sha>)
curl -s http://pamv1:8080/metrics | grep pam_build_info      # same, for monitoring
```

**Status:** **[v0.65.1](https://github.com/morandeirachema/pamv1/releases/tag/v0.65.1)
was released on 2026-09-03** and is what every manifest here pins — image digest
`sha256:4274ec4e28ec8ab5f9a89c2bd1d1389bf642c50c01a04b28f7189b67effcc52e`, public
