...
>
> Last updated: 2026-08-27 · Reflects: Phases 0–221. Phases 217–220 add no requirement: one additive migration (`0050`, a small self-purging table for the agent-budget reservation, applied at startup on the PostgreSQL already required) and no knob, dependency, listener or external service. Phase 189 adds no requirement either: one additive index-only migrations (`0047`, two plain indexes on grant tables the schema already has, `0048`, one additive nullable column, and `0049`, Phase 212's partial unique index on active agent-key names) and one new knob defaulted to today's behaviour, dependency, listener or external service. Phases 161–183 add no requirement: three additive migrations applied at startup (`0044`, `0045`, `0046`) on the PostgreSQL instance already required, five optional knobs (`PAM_BROKER_MAX_RESULT_BYTES` 165, `PAM_BROKER_BUDGET_PER_DAY` 167, `PAM_BROKER_REQUIRE_ENROLLED_SVID` 174, `PAM_BROKER_REQUIRE_KNOWN_OWNER` 176, `PAM_BROKER_POSTURE_REQUIRED` 180 — the last needing the posture webhook this deployment may already run for human operators, never a new system), and no new dependency, listener or external service. Phases 66–70 add one more
