...
- **RADIUS login** — if your organization routes sign-in through a RADIUS
  server (a token gateway, say), fill *User* and *Password* as for AD; the
  portal may then ask for a **one-time code** from your token — type it and
  you are in. Where RADIUS is the **second factor**, put the code from your
  token in the *MFA code* field alongside your directory password (Phase 269).
...
| 32 | Work with label rules — allow / deny access by target label | read_inventory (changing needs `manage_targets`) |
| 33 | Join a shared desktop — redeem a session invite for an RDP/VNC session in the portal | any |
| 34 | Work with session probes — what an operator's Windows RDP session is running and connecting to, and the block rules a probe enforces inside it (Phase 266) | read_audit (rules and *End process* need `manage_targets`) |
| 90 | Sign off | |
...
On some Windows servers your administrator runs a **session probe** inside
your RDP session (Phase 266). It runs as you, with your permissions and no
more: it reports which programs your session runs and where they connect,
and it ends a program that a block rule names — a blocked tool simply
closes, and the audit trail says which rule closed it. It cannot do anything
you could not do yourself, and it never sees your password.
...
| Date | Change |
|---|---|
| 2026-09-22 | Phase 276: your organization may show a **notice** on the Sign On screen and before an SSH session prompt, and a **recording notice** when a session opens — printed at the top of an SSH session, and as a dialog you must accept before a desktop opens (Cancel opens nothing). Acknowledging is on the record. |
| 2026-09-22 | Phase 275: a command may be refused with `PAMv1: session ended by a restriction rule` — a rule your administrator set on you or your role ended the session; an SFTP transfer past a size limit is refused the same way. Both are audited with the rule; ask your administrator. |
| 2026-09-22 | Phase 274: an approver may now leave a **comment** on your request (and may be required to), may grant **less time than you asked** (never more), and may **cancel** an approved request — your session on that target ends at once, with the reason on record. A request nobody decides within your organization's timeout expires on its own; file it again. Approvers: options 5 and 6 prompt for a comment (and 5 for a duration), option 9 cancels an approved request. |
| 2026-09-22 | Phase 272: a connection refused with `host key mismatch for <target>` means the server answered with a different SSH key than the one PAMv1 pinned on first contact — possibly a re-installed host, possibly not. It is not yours to override: tell your administrator, who can see the pin (*Work with Targets* → 12) and reset it if the change was expected. |
| 2026-09-22 | Phase 271: on a server with the session probe, which programs you start and which window is in front are now part of the session's record — auditors see them beside the desktop recording (kind `probe` under *Session recordings*) and can search them; a rule may also just **notify** rather than close a program. |
| 2026-09-22 | Phase 270: a target (or your grant to it) may now allow only some of what its protocol can do — `pamv1: ssh_exec is not granted for this target` means your shell is fine but commands are not, and a desktop may open with no drive, printer or microphone; ask your administrator, the refusal is audited with the name of the missing right. |
| 2026-09-22 | Phase 269: the Sign On screen may ask for a **one-time code** after your password (RADIUS challenge), or take it in the *MFA code* field when RADIUS is your second factor. §3 |
| 2026-09-22 | Phase 267: the menu table now lists **32** (label rules) and **33** (join a shared desktop), which had shipped unlisted, alongside the new **34**. |
| 2026-09-22 | Phase 266: on a Windows server where your administrator runs the **session probe**, your RDP session's programs and connections are visible to auditors (menu 34; *Work with Active Sessions* → 8) and a program a block rule names is closed — with your own permissions, never more. §"Connecting to a remote desktop" |
