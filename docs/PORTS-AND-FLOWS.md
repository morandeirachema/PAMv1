...
>
> Last updated: 2026-09-02 · Reflects: Phases 0–227 and 229–265. **Phase 53 added the first new
...
| I8 | **Endpoint agent** (`pam-agent`, on a NAT'd/firewalled target — target zone, or wherever the endpoint sits) | pam-server | 2222 | SSH | Outbound-only reverse tunnel: authenticates as `endpoint-agent:<name>` with its own bearer key, requests one `tcpip-forward`, then only carries streams pam-server opens toward it. Replaces E2 for that target (Phase 153). A probe-kind agent (Phase 266) rides the same flow from inside an operator's Windows logon session, carrying telemetry and block-rule enforcement instead of a tunnel — no new port, listener or egress | ✅ P153, P266 |
