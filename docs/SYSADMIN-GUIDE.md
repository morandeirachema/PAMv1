...
| Date | Change |
|---|---|
| 2026-08-15 | Phase 145 (generic file-attachment secrets): new `secret_type: "file"`, capped by new env var `PAM_CREDENTIAL_FILE_MAX_KB` (default 1024). No new background worker, no migration. Caught and fixed a near-miss along the way: the plan's own fix for `ListCredentials`' cost would have broken the PostgreSQL proxy's JIT credential injection in production — see ROADMAP.md Phase 145 |
| 2026-08-15 | Phase 143 (ICAP-based file-transfer scanning): new env var `PAM_ICAP_URL`, off by default; requires SFTP capture already on and size-capped. No new background worker, no migration |
