...
| (e) | Security in acquisition/development/maintenance, vuln handling | Versioned DB migrations; CI gates `gofmt`/`vet`/`staticcheck`/`govulncheck`/`gosec`/`test -race`; SBOM + cosign-signed releases + SLSA provenance (`.github/workflows/release.yml`) | 🟡 partial — **the pipeline exists and is test-gated, but no release has been cut** (zero tags), so nothing has actually been signed or attested. A workflow file is not evidence for a supply-chain control |
