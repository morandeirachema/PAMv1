...
>
> Last updated: 2026-08-27 · Reflects: Phases 0–218 + the 2026-07 hardening
...
|---|---|---|
| 2026-08-27 | **Phase 215 — the 2026-08-27 audit: six findings, all closed** (`SECURITY-AUDIT-2026-08-27.md`). Two HIGH: a deleted/deactivated/re-roled user kept every session it already held (a 24-hour extension token among them), and a web guest's bearer key was served to supervisors as its roster `join_id` and written to the kick audit. One MEDIUM: session-token principals never carried the user's IP allowlist or device binding. Three LOW: the `authenticated` middleware and the viewer tunnel ran no source gates; a delegated manager could rewrite a personal safe's contents. Same two shapes as the previous audit — a check re-implemented with fewer fields, and now its sibling, state derived once from a row and never re-derived |
| 2026-08-26 | **Phase 214 — supply chain: a release tag published an image nothing verifies.** The repository was renamed `PAMv1` alongside Phase 213 and `release.yml` built its image reference from `github.repository`; the push succeeded (`docker/metadata-action` lowercases the tags it emits) and the SBOM step refused the uppercase reference, so `0.58.0` exists in GHCR unsigned, unattested and without a GitHub Release. Same class as AW (the pipeline's own environment changed under an unchanged file) and the same shape as Phase 192 (an artifact produced by a path no check had exercised — the rehearsal rule keyed on a file diff and the file had not changed). Closed by v0.58.1: the name is lowercased once and an uppercase result fails the job, in the rehearsal too; the 0.58.0 tag stays (Go proxy cached) and the CHANGELOG says not to deploy it |
| 2026-08-25 | **Phase 203 — closed: a target nobody restricted was reachable by everyone who could connect.** Deferred in writing by Phase 189 and rendered red by the reachability review ever since, because it is an estate-wide default rather than a decision about any particular system. `PAM_REQUIRE_TARGET_GRANT=true` now makes such a target reachable by nobody until it is granted. Left **default-false** deliberately: turning it on is a real change for a running estate, and the migration path is the review itself — read menu 31, see the `open` count, grant those targets, then set the flag. Admins still bypass, which is an explicit decision about a role rather than an omission |
| 2026-08-25 | **Phase 201 — an alias could be planted on another application's grant.** `POST /v1/apps/{id}/grants/{gid}/alias` parsed only `{gid}`, so naming a grant under one application renamed a different application's grant and answered `200` — handing that app a stable, git-committable name for a credential nobody meant to expose, while the operator read success. Needs `CapRevealSecret`, so it is a privileged operator's mistake landing silently on the wrong object rather than an unprivileged escalation; shipped in v0.54.0 and fixed here, scoped like its sibling `deleteAppSecretGrant` and guarded by a test verified against the unfixed handler. Same phase: the Phase 197 store contract was discovered never to have run on either backend, and `archgen`'s `rateLimit(` needle was swallowing the auth wrapper nested inside it, republishing both WebAuthn login routes as unauthenticated — the exact failure Phase 195 existed to prevent |
