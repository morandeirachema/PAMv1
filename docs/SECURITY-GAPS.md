...
>
> Last updated: 2026-07-26 · Reflects: Phases 0–37 + the 2026-07 hardening passes.
...
| C | **Command control does not cover every path with a discrete command.** `CommandGuard` is consulted only in `internal/proxy`; `POST /api/targets/{id}/winrm` and the broker's `ssh_exec`/`winrm_exec` tools take a command string and never check the denylist. | A pattern blocked for a human on the SSH proxy runs freely through the REST WinRM endpoint (any `CapConnect` holder) and through the agent broker — the least-trusted actor. Phase 16 documents the guard as exec-path-only *within the proxy*; the claim of "every path where a discrete command is visible" does not hold outside it. | Lift `CommandGuard` out of `internal/proxy` and apply it in `execWinRM` and the broker exec tools, auditing `command.blocked` identically. |
