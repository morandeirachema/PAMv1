...
`rdp.error` · `authz.denied` · `login.failed` · `api.auth_failed` · `proxy.auth_failed` · `proxy.auth_rate_limited` · `session.revoked` · `session.killed` · `vault.kek_rotated` · `breakglass.access` · `session.start` ·
