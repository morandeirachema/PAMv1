...
numbered migration — `0023_certification_four_eyes.sql` at time of writing), applied under a
...
| `PAM_RDP_CLIPBOARD` | `allow` | RDP clipboard policy: `allow`, `readonly` (block paste into the target), `deny` (clipboard off both ways); drive redirection always off (Phase 33) |
| `PAM_RDP_CLIPBOARD_AUDIT` | `off` | audit what crosses the clipboard bridge (Phase 50): `meta` (direction, mimetype, bytes, SHA-256) or `full` (also the content, truncated — the clipboard may hold a just-copied password, so this is opt-in); emits `rdp.clipboard` |
...
`mfa.recovery_used` · `session.kill_failed` · `winrm.run` · `winrm.error` · `winrm.refused` · `ssh.exec` · `rdp.token` · `rdp.connect` · `rdp.end` · `rdp.refused` · `session.self_stepup_denied` ·
`rdp.error` · `rdp.clipboard` · `authz.denied` · `login.failed` · `api.auth_failed` · `proxy.auth_failed` · `proxy.auth_rate_limited` · `session.revoked` · `session.killed` · `vault.kek_rotated` · `breakglass.access` · `session.start` ·
...
`sftp.session` · `sftp.open` · `sftp.modify` · `sftp.blocked` · `sftp.denied` · `sftp.parse_error` · `session.subsystem` ·
...
`certification.campaign_created` · `certification.item_certified` · `certification.item_revoked` · `certification.decision_denied` · `certification.campaign_closed` ·
`access.ticket_rejected` · `access.approve_partial` · `access.consumed` ·
