...
>
> Last updated: 2026-09-22 · Reflects: Phases 0–227 and 229–275. **Phases 206–207 change nothing here**: proof of possession adds no table, no migration and no artifact to back up — a `cnf` thumbprint is a claim inside an already short-lived token, and the key it names is held by the agent, never by PAMv1. The procedure is unchanged, and the migration high-water mark is now **`0066`**: Phase 275 added it (`restriction_rules`, a policy table — patterns and sizes, never a secret; a plain dump covers it). Before it, `0065`: Phase 274 added it (`access_requests.notes`, the approvers' comments — one additive text column, never a secret). Before it, `0064`: Phase 272 added it (`target_host_keys` — the SSH host key each target presented on first contact, public material, never a secret; restoring an older dump restores older pins, which a re-keyed host then fails against until reset, and a dump with none re-pins on the next contact — nothing extra to back up). Before it, `0063`: Phase 271 added it (`probe_rules.action`, one additive column defaulting to `kill`; never a secret). Its other artifact is a FILE: each probe session's `.probe.log` in the recording directory, sealed under the recording key when recordings are — so the same key-inventory bullet as `.cast` covers it, and a dump that omits the recording directory loses it exactly as it would a recording. Before it, `0062`: Phase 270 added it (`targets.rights` and `target_grants.rights`, two additive text columns defaulting to empty — an authorization input, never a secret; a plain dump covers them). Before it, `0061`: Phase 266 added it (`endpoint_agents.kind` — the one-live-per-target index re-keyed per (target, kind) — and `probe_rules`, the block rules a session probe enforces; neither carries a secret, a restore carries them like any other rows, and a probe's telemetry is never persisted, so there is nothing extra to back up). Before it: `0060` (Phase 264, `access_requests.approved_as`), `0059` (256, approval tiers), `0058` (252, credential-level grants), `0057` (250, target labels + label rules), `0056` (246, safe permission sets), `0055` (244, per-session MFA), `0054` (242, identity lock + token expiry), `0053` (240, grant lifetime), `0052` (236, `users.slack_user_id`) — all additive columns, indexes or plain tables, none holding key material; a plain dump covers each. Before those, `0051`: Phase 222 added it (one additive `subject` column on `broker_tokens` — the identity a resume token is bound to; a restore carries it like any other column, and a token restored without one — a pre-migration row — merely spends for anyone for its remaining minutes, so there is nothing extra to back up). Before it, `0050`: Phase 219 added it (`agent_call_reservations`, the compare-and-spend ledger behind the agent budget — a transient table holding at most one rolling 24-hour window per agent and purging itself on write; a restore carries it like any other table, and restoring an older dump only means an agent is under-served for at most a day, never over-served, so there is nothing extra to back up). Before it, `0049`: Phase 212 added it (one partial unique index, `agent_keys_active_name_unique` — at most one ACTIVE agent key per name, the 2026-08-26 audit's M-3; an index carries no data, so a restore rebuilds it and there is nothing extra to back up — **but a dump taken before v0.58.1 that holds two active agent keys with the same name will not start under v0.58.1 until one of them is disabled**, see the CHANGELOG's upgrade note). Before it, `0048`: Phase 197 added it (one additive nullable
...
| Date | Change |
|---|---|
| 2026-09-22 | Phase 275 (restriction rules): migration high-water mark `0065` → `0066` (`restriction_rules`). No new key-inventory bullet. |
| 2026-09-22 | Phase 274 (approval depth): migration high-water mark `0064` → `0065` (`access_requests.notes`). No new key-inventory bullet. |
| 2026-09-22 | Phase 272 (host-key pins): migration high-water mark `0063` → `0064` (`target_host_keys`, public keys only). No new key-inventory bullet. |
| 2026-09-22 | Phase 271 (probe metadata): migration high-water mark `0062` → `0063` (`probe_rules.action`); the `.probe.log` artifacts live in the recording directory under the recording key — covered by the existing recordings bullet, nothing new to inventory. |
| 2026-09-22 | Phase 270 (sub-protocol rights): migration high-water mark `0061` → `0062` (`targets.rights`, `target_grants.rights`), additive text columns; no new key-inventory bullet. |
| 2026-09-22 | Phase 267 (documentation currency pass): the header's migration high-water mark had sat at `0051` through nine migrations; now `0061`, with 0052–0061 attributed to their phases. Phase 266's `endpoint_agents.kind` + `probe_rules` carry no secret and probe telemetry is memory-only — no new key-inventory bullet. |
