...
its release page, and at the top of the release notes themselves, with the exact
`cosign verify` command for that image.
...
| `PAM_SSH_KNOWN_HOSTS` | | | OpenSSH known_hosts file pinning **upstream target** host keys; authoritative when set. |
| `PAM_SSH_HOST_KEY_CHECK` | | `tofu` | (Phase 272) Without a known_hosts file: `tofu` pins each target's SSH host key on **first contact** and refuses a later mismatch until an administrator resets the pin (*Work with Targets* → 12, F6); `strict` also refuses a target that has no pin yet; `off` trusts any key (logged loudly — the pre-272 behaviour). |
...
| `PAM_APPROVAL_TIMEOUT_MIN` | | `0` | (Phase 274) Expire a **pending** request nobody decided within this many minutes (`0` = never). |
| `PAM_APPROVAL_COMMENT_REQUIRED` | | `false` | (Phase 274) Require the approver's comment on every approve, deny and cancel. |
...
2. Install `pam-agent_windows_amd64.exe` (a Release asset, or
   ...
   sessions probing.
3. Add block rules — menu 34, F10, F6 (`POST /api/probe-rules`). A rule's
   **action** is `kill` (end the process — the default) or `notify` (Phase
   271: report the match as `probe.rule_notified` and leave the process
   running — the tripwire without the trip, for tools you want to see used
   rather than stop).

**What the session did, on record (Phase 271).** Beside the desktop
recording, every probe session writes a **metadata artifact**
(`<timestamp>_<target>_<account>.probe.log` in the recording directory):
one JSON line per event — each process that started (with its command
line) or ended, each change of the foreground window (title and owning
process), and each rule match. It is sealed under the recording key when
recordings are, hashed and appended to the recording hash chain, and
audited `probe.record file:… sha256:… chain:…` when the probe leaves.
*Session recordings* (menu 19) lists it as kind `probe`, option 5 shows it
as text, and the content search (F4) finds it — "which session ran
`psexec`" is one query across desktops and terminals alike. A probe whose
artifact cannot be opened is refused: telemetry that leaves no record is
what this exists to prevent. The metadata events are not audit rows (a
busy desktop starts hundreds of processes); only enforcements and notify
matches are.
...
an API decision's do (Phase 238). A click passes the **user-keyed** gates
an API call passes — device posture and on-call attestation, refused with
the same `authz.denied` reason — but **not** the two bound to the caller's
origin: the request reaches PAMv1 from Slack's servers, so the approver's
**IP allowlist and enrolled device cannot apply** to a Slack decision. If
either of those is what protects an approver's account, do not link that
account to Slack. Buttons posted by a PAMv1 older than 0.65.0 no longer
verify (the token MAC is domain-separated since then): re-run
`slack-notify` for any request still pending across that upgrade.
...
### Approver comment, granted duration, cancel and timeout (Phase 274)

What an approver can say and do beyond yes/no — WALLIX's approval tab,
minus nothing that a proxy can honour:

- **A comment.** `POST /api/access-requests/{id}/approve` and `/deny` take
  an optional body `{"comment": "…"}`; the console prompts for it on options
  5 and 6. It is kept on the request (`notes`, one line per approver) and
  written to the audit row. `PAM_APPROVAL_COMMENT_REQUIRED=true` makes it
  mandatory everywhere.
- **A shorter grant.** `{"duration_min": 15}` on the approve that completes
  the chain grants fifteen minutes from now — **capped at what was
  requested**: an approver can shorten a window, never extend it. The audit
  row carries `granted_until:`.
- **Cancel an approved request.** `POST /api/access-requests/{id}/cancel`
  (`approve` capability; console option **9**) with a mandatory reason: the
  request becomes `cancelled`, admits nothing further, and **every live
  session the requester holds on that target is ended** (the trail says how
  many, `access.cancel … sessions_killed:`; the alerter is told). Four-eyes
  applies: the requester cannot cancel their own approval.
- **A timeout.** `PAM_APPROVAL_TIMEOUT_MIN=120` expires any request still
...
### Host keys pinned on first contact (Phase 272)

Unless `PAM_SSH_KNOWN_HOSTS` pins them from a file, PAMv1 now remembers the
SSH host key each target presents the **first** time the proxy reaches it
and checks every later connection against it — WALLIX's "server pubkey
store" with its check modes, and OpenSSH's own trust-on-first-use, applied
per target row:

- **The first contact pins.** `target.hostkey_saved` records the key type
  and SHA256 fingerprint, and the alerter (webhook/syslog/email) receives
  `hostkey.saved`. Compare it with the host's own
  `ssh-keygen -lf /etc/ssh/ssh_host_*_key.pub` if you want the first contact
  verified by hand.
- **A different key refuses the session.** The operator's connection fails
  with `host key mismatch for <target>: presented …, pinned …`, the trail
  records `target.hostkey_mismatch` with both fingerprints under the
  operator's actor, and the alerter receives `hostkey.mismatch`. The pin is
  **never re-learned** from a mismatch: that is the whole point.
- **A re-keyed host is reset on purpose.** *Work with Targets* → **12**
  shows the pin (type, fingerprint, first and last seen, the public key
  line); **F6** resets it, or `DELETE /api/targets/{id}/host-key`. The reset
  is audited with the fingerprint it dropped (`target.hostkey_reset`), and
  the next connection pins the key it sees.
- **`PAM_SSH_HOST_KEY_CHECK=strict`** refuses a target with no pin at all
  (`target.hostkey_unknown`) — seed pins by connecting once under `tofu`,
  or through the API, then switch.
- **`off`** restores the pre-272 trust-any behaviour, with a loud warning.

Scope: the proxied SSH sessions (including those through an endpoint
agent's tunnel — the pin is the endpoint's own sshd key). The management
connections PAMv1 makes itself — password rotation, account discovery,
post-session forensics — keep the known_hosts-or-trust-any rule. RDP cannot
be pinned this way: the Guacamole daemon terminates the RDP TLS and PAMv1
never sees the server certificate (`PAM_GUACD_IGNORE_CERT` governs guacd's
own verification).

### Sub-protocol rights — what a session may do inside its protocol (Phase 270)

A grant says *who* may reach a target; **rights** say *what they may do once
there*, the way WALLIX's per-service proxy options and per-authorization
sub-protocol lists do. Nine names:

| Right | Protocol | Allows |
|---|---|---|
| `ssh_shell` | SSH | an interactive shell |
| `ssh_exec` | SSH | a remote command (`ssh host cmd`; SCP rides on it) |
| `ssh_sftp` | SSH | the SFTP subsystem |
| `ssh_forward` | SSH | client-initiated port forwarding (`ssh -L`, to the target's own host) |
| `ssh_x11` | SSH | X11 forwarding |
| `rdp_drive` | RDP | drive redirection (files through the desktop) |
| `rdp_printer` | RDP | printer redirection |
| `rdp_audio` | RDP | audio output |
| `rdp_audio_in` | RDP | audio input (microphone) |

- **On a target** (`rights` on *Work with Targets* → F6/2, or `POST/PUT
  /api/targets`): a comma list; blank means everything the deployment allows,
  which is what every existing target has. `ssh_shell,ssh_sftp` on a
  file server means nobody runs commands there, whatever their grant says.
- **On a grant** (`rights` on *Work with grants* → F6, or `POST
  /api/targets/{id}/grants`): narrows the target's set for that subject.
  Several admitting grants add up; a matching grant with no set leaves the
  target's set as it is. A grant that narrows to nothing the target allows
  admits the session to nothing (an explicit refusal, never "everything").
- **The deployment's switches are ceilings.** `PAM_SSH_PORT_FORWARD=false`,
  `PAM_SSH_SFTP_MODE=deny` and the four new desktop ceilings —
  `PAM_RDP_DRIVE`, `PAM_RDP_PRINTER` (both default `false`, as drive
  redirection always was: a file through a redirected drive leaves no
  record, SFTP is the audited path), `PAM_RDP_AUDIO` (`true`),
  `PAM_RDP_AUDIO_IN` (`false`) — cannot be turned on by a row. A right only
  narrows.

A refused request answers the client with failure, writes `pamv1: <right> is
not granted for this target` into the recording, and audits
`session.right_denied target:… cred_user:… right:…`. The desktop viewer
sends every guacd redirection switch explicitly from the same decision.
Clipboard keeps its own per-target policy (`rdp_clipboard`); the command
denylist still applies to an admitted exec.
...
### 9.1a Login banner and recording consent (Phase 276)

Two texts a privacy regime expects a PAM to show, WALLIX's *Connection
messages*, per language:

- **The login banner** (`PAM_BANNER_LOGIN`) is shown **before**
  authentication: SSH clients print it before the password prompt (the
  RFC 4252 pre-auth banner), and the portal shows it on the Sign On
  screen (`GET /api/banner` is public for exactly that reason; it exposes
  the two texts and nothing else).
- **The session notice** (`PAM_BANNER_SESSION`) is the recording
  acknowledgement. On an SSH session it is printed to the operator before
  anything from the target and written into the recording, so the `.cast`
  opens with it; audited `session.consent … mode:printed`. On an RDP/VNC
  desktop there is no stream to print into, so the portal asks first: the
  token request is answered `428` with the text until the operator clicks
  OK, and only then is a token minted — audited `session.consent …
  mode:acknowledged`. A refusal mints nothing. Both rows carry
  `banner_sha256:`, so the trail names WHICH text was shown.

Per-language variants: `PAM_BANNER_LOGIN_ES`, `PAM_BANNER_SESSION_FR`, and
so on (the portal sends the browser's language; `es-MX` falls back to `es`,
then to the default; the SSH proxy always uses the default). `\n` in a value
...
### 9.4a Per-user and per-role restriction rules (Phase 275)

Command control (`PAM_COMMAND_DENY_FILE` / `_ALLOW_FILE`) is one policy for
everyone. A **restriction rule** belongs to a subject — a user, or a role
everyone holding it — and says what *they* may not do inside a session, the
way WALLIX's user-group Restrictions tab does. Menu **35** (*Work with
restriction rules*), or `POST /api/restriction-rules`:

| Field | Values |
|---|---|
| `subject_type` / `subject` | `user` + a username, or `role` + `admin`/`user`/`auditor`/`approver`/`agent` |
| `subprotocol` | `ssh_exec`, `winrm`, `sql`, `kubernetes`, `sftp`, or `*` (any command path) |
| `pattern` | a regular expression over the command; on `sftp`, a size rule — `$filesize:>10m` (an upload larger than 10 MiB), `$downsize:>100m` (a download) |
| `action` | `kill` — end the session (or refuse the call where there is no session); `notify` — record the match and let it through |

A user's own rules and the rules of every role they hold add up; when a
kill rule and a notify rule both match, kill wins. The set is loaded **when
the session is admitted**, so a running session keeps the rules it started
under and a new rule takes effect on the next one. Every match is audited —
`restriction.notified` / `restriction.killed` with the rule and the quoted
pattern, `sftp.size_limit` with the bytes and the limit — and a kill
refusal reaches the operator as `PAMv1: session ended by a restriction
rule`. Size rules count bytes per file handle: the first write or read that
crosses the limit is the one refused. Like command control, this is not a
...
| 2026-09-22 | **Phase 272 (host-key pins).** §6 new subsection *Host keys pinned on first contact*: the tofu/strict/off modes, what a mismatch looks like, resetting a pin (Targets → 12, F6), the scope (proxied SSH; not rotation/discovery, not RDP). Env row `PAM_SSH_HOST_KEY_CHECK`; the `PAM_SSH_KNOWN_HOSTS` row no longer says trust-any. |
| 2026-09-22 | **Phase 271 (probe metadata).** §6 *Windows session probes*: rule actions (`kill`/`notify`) and the per-session `.probe.log` metadata artifact — what it records, how it is sealed and chained, where it lists and searches. |
| 2026-09-22 | **Phase 270 (sub-protocol rights).** §6 new subsection: the nine rights, on a target and on a grant, deployment switches as ceilings, what a refusal looks like; four new env rows. |
