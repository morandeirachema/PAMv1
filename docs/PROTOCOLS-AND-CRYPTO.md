...
| Login sessions, agent keys, app keys, broker resume tokens | high-entropy random; only the SHA-256 stored; resume tokens single-use and, since Phase 222, bound to the subject that parked the call (refused for any other presenter at peek and spend) | hash lookup |
...
| Session-share invite token / guest key (Phase 116) | Invite token: high-entropy random, **only the SHA-256 stored** (`auth.TokenHash`), single-use. Guest key (minted on redemption): 32 bytes from `crypto/rand`, hex-encoded — kept in memory only, never written to the database, scoped to one replica, and **stored and looked up by its SHA-256** (Phase 212; the roster's `join_id` is that hash, never the key — Phase 215) | hash lookup (token) / in-memory hash lookup (guest key) |
| DoubleLock password (Phase 135; hardened Phase 212) | never stored. A 16-byte random salt, then PBKDF2-HMAC-SHA-256 derives a **verifier** (`password+"\|verify"`) that is stored and an AES-256-GCM **key** (`password+"\|key"`) that seals `DoubleLockEnc`, a copy of the secret deliberately outside the KEK. **600 000 iterations** since Phase 212 (was 100 000), the count stored per record as an `i=<n>;` prefix so older values open at the count they were sealed with; minimum password length 16, raisable by `PAM_DOUBLELOCK_MIN_LENGTH`, because entropy — not iteration count — is what defeats the offline attack this KEK-independent copy invites | verifier compared in **constant time** (`crypto/subtle`) |
...
### 2.8 Token verification (five independent verifiers)
...
| **RFC 9449 proof of possession** (`internal/agentid/pop.go`, Phase 206) | RS256, ES256 (P-256), **EdDSA** — the same closed asymmetric set, reusing `verifySignature`, so `none` and every symmetric alg are refused by construction rather than by a denylist | `typ` must be `dpop+jwt`; the embedded `jwk` must carry **no private material**; `htm`/`htu` must match this request (origin from `PAM_BROKER_PUBLIC_URL`, never `X-Forwarded-*`); `iat` within 60 s either way; `ath` must be the SHA-256 of the token actually presented; the JWK's [RFC 7638](https://datatracker.ietf.org/doc/html/rfc7638) thumbprint must equal the token's `cnf.jkt` (constant-time); and the `jti` must be unseen — a proof is **single-use within its freshness window on the replica that saw it** — the replay cache is in-process, so a multi-replica deployment accepts one replay per replica inside the ±60 s window (RFC 9449's server `nonce` is the standard's answer and is not implemented) — with a per-key quota that fails closed rather than evicting |

All five make failures **indistinguishable** to the caller — no oracle telling an
...
- **`may_act` is now issued as well as enforced (Phase 181).** `exchange.go` has
  ...
  subject itself, and recorded on the trail.
- **A minted token can now be bound to a key (Phase 206).** Until this phase a
  delegated token was a **bearer** credential — anything that captured one was
  that sub-agent until it expired, with its TTL (narrowable per rule since Phase
  171) as the only bound. The exchange now accepts `cnf_jkt` — again a PAMv1
  extension parameter, since [RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693)
  defines no such field and [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449)'s
  own binding flow has the *client* prove its key at the token endpoint, which
  cannot apply when the caller is the delegator — and stamps an
  [RFC 7800](https://datatracker.ietf.org/doc/html/rfc7800) `cnf: {jkt: …}`
  confirmation. Every call presenting that token must then carry a `DPoP` proof
  header, verified as in the table above. Three rules make the binding hold
  rather than merely exist: a **bound delegator may not mint an unbound token**,
  so the constraint cannot be walked off one hop down; a `cnf` this verifier
  cannot enforce (RFC 7800 also defines `jwk` and `kid` confirmations) **refuses
  the token** rather than reading as "unbound", which would downgrade what an
  issuer deliberately constrained; and `Authorization: DPoP <token>` is accepted
  alongside `Bearer` but the **scheme is never consulted** — the `cnf` claim
  inside the token, not a header word the caller chose, decides whether a proof
  is demanded. **What it does not establish**: the thumbprint is named by the
  DELEGATOR, so PAMv1 cannot attest that the bound key belongs to the sub-agent
  rather than to the delegator itself. The property gained is that a token
  lifted off the wire or out of a log is useless without the key — theft, not
  attestation, which stays SPIRE's job.
...
| **PostgreSQL wire** | `PAM_DB_ADDR`, off (`:5433` by convention) | only with a cert | `SSLRequest` answered `S`/`N`; **GSSEnc always refused**; message bodies bounded at **64 KiB before authentication** and 64 MiB in session (Phase 212 — `pgproto3`'s default is no bound), with the connection rate limiter ahead of the first read |
...
| PBKDF2-HMAC-SHA-256 | SCRAM salted password; DoubleLock verifier + key (600 000 iterations, count stored per record) |
...
| Date | Change |
|---|---|
| 2026-08-26 | **Phase 206 — proof of possession: a fifth JWT verification path (§2.8), one new claim, one new request header, no new port.** A delegated token minted by the broker was a bearer credential; it may now carry an [RFC 7800](https://datatracker.ietf.org/doc/html/rfc7800) `cnf: {jkt: …}` naming the [RFC 7638](https://datatracker.ietf.org/doc/html/rfc7638) thumbprint of a key, and every call presenting it must send an [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449) `DPoP` proof signed by that key. The verification reuses the SVID path's own `publicKeyFromJWK` and `verifySignature`, so there is one algorithm set rather than two that can drift; the canonical JSON hashed for a thumbprint is **built by hand** because RFC 7638 specifies exact octets and an encoder may legally produce different ones, with every interpolated member checked to be base64url first (a value carrying a quote could otherwise forge the canonical form of a different key). SHA-256 appears twice more here, both times as an identifier rather than a secret: the thumbprint, and `ath` (the hash of the presented token, which binds a proof to the one token it was made for). No new port, no new secret class — a thumbprint is a hash of a PUBLIC key and is recorded on the audit trail deliberately — and no new key-derivation primitive. `PAM_BROKER_PUBLIC_URL` exists because `htu` is compared against the origin the *client* signed, which is not the one a request arrives on behind a TLS-terminating proxy; `X-Forwarded-*` is never consulted, since a caller choosing what its own proof is checked against would remove the check |
...
| 2026-08-27 | **Phase 215 (the second audit).** §2.5's guest-key row said "not hashed" three phases after Phase 212 hashed it; it now says hashed, and that the roster's `join_id` is the hash, never the key (before Phase 215 the roster served the raw key). Nothing else here changed: the audit read the vault envelope, DoubleLock, the JWT/DPoP verifiers, the OIDC/SAML callbacks and the token exchange clean |
| 2026-08-26 | **Phase 212 (the audit) — DoubleLock at 600 000 iterations with a minimum length; the PostgreSQL proxy's message-body bound.** §2.5 gains the DoubleLock row its Phase 135 entry below says it had (verifier/key derivation, the per-record iteration count, the length floor and why length is the real defence); §5's inventory names it. §3.1: the `pgproto3` backend is bounded — 64 KiB before authentication, 64 MiB in session, where the library default is no bound — and the rate limiter runs before the first read (C-1). Also this phase: the upstream SCRAM iteration count is bounded at 600 000; channels per authenticated SSH connection capped at 64; `SSH_FXP_LINK`/`SSH_FXP_SYMLINK` check both paths in every SFTP mode; the token exchange verifies the actor token's `cnf` |
