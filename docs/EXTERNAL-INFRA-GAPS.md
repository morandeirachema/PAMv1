...
|---|---|---|---|---|
| **RADIUS login / second factor** (Phase 269) | `PAM_RADIUS_*`, `internal/radius`, `internal/auth/radius.go` | `radiustest`, a real in-process RFC 2865 server (PAP recovered, replies signed) | A FreeRADIUS / NPS / token gateway accepting PAP | Accept, reject, a challenge for a code, and a wrong shared secret (the reply is ignored) behave as the tests say |
...
| Capability | Env / code | CI proof | You must provide | Verify |
|---|---|---|---|---|
| **Windows session probe — the platform layer** (Phase 266) | `PAM_AGENT_MODE=probe`, `internal/probe/platform_windows.go` (PowerShell `Get-CimInstance Win32_Process` / `Get-NetTCPConnection`, `TerminateProcess`) | The protocol, enforcement loop, hub, proxy branch and API are proven with the real agent library against the real proxy and a scripted logon session; the Windows file compiles and vets under `GOOS=windows` | A Windows Server with an RDP target and the probe launched at log-on as the operator | The session's processes and connections appear on menu 34; a process rule ends a matching program; a process another user owns is refused (`access is denied`) |
...
| Capability | Env / code | CI proof | You must provide | Verify |
|---|---|---|---|---|
| **SPIFFE JWT-SVID verification** | `PAM_BROKER_TRUST_DOMAIN_JWKS`, `internal/agentid/svid.go` | file JWKS + signed SVIDs, rotated on disk | A trust-domain JWKS (from SPIRE or another issuer) | SVIDs validate (subject/audience/exp); RFC 8693 `act` delegation depth is capped; the bundle file is re-read when it changes (Phase 224), so a rotation needs no restart |
