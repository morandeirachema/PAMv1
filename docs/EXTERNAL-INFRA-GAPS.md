...
be closed by infrastructure rather than code. A related in-process item —
enrolling SVID identities so that only known ones may call at all, with
first-seen/last-seen — is deliberately **not** here: it is buildable without a
SPIRE deployment and is tracked in
