...
>
> Last updated: 2026-09-22 · Reflects: Phases 0–227 and 229–276 + the 2026-07 hardening passes.
>
...
| 2026-09-22 | Phase 276 (banners): `internal/banner` (`Banners`, `New`, `Get`, `Has`, `Digest`), `config.bannersFromEnv`, `proxy.Config.Banners` + `BannerCallback`, `api.Options.Banners`, `getBanner`, the consent gate in `viewerToken`. |
| 2026-09-22 | Phase 275 (restriction rules): `internal/restrict` (`Validate`, `Load`/`Compile`, `Set.Check`, `Set.SizeLimit`, `Subject`), `admitResult.restrictions`, `handleSession(…, rs)`, `winrmRun(…, rs, killSession)`, `sqlBlockedStatement(…, rs)`, `sftpInspector.setSizeLimits`/`overSize`, `Proxy.killRegistered`, `api/restriction_handlers.go` (`checkRestriction` behind `guardCommand`). |
| 2026-09-22 | Phase 274 (approval depth): `approvalDecisionIn`/`readDecision`/`commentDetail`, `decideAccessRequestWith`, `cancelAccessRequest`, `expirePendingAccessRequests`/`SweepApprovalTimeouts`; store `NoteAccessRequest`, `ShortenAccessRequest`, `CancelAccessRequest`, `ExpirePendingAccessRequests`; `AccessRequest.Notes`. |
...
| 2026-09-17 | Phase 256 (tiered approval): `internal/store/approvaltiers.go` (`ApprovalTier`, `TierState`, `ParseApprovalTiers`, `NormalizeApprovalTiers`, `JoinApprovalTiers`, `HasManagerTier`, `TierProgress`), `Target.ApprovalTiers`, `Safe.ApprovalTiers`, `User.Manager`, `ApprovalPolicy.Tiers`, `AccessRequest.Tiers`, `UserStore.UpdateUserManager`; `api.approvalTiersForTarget`, `tierQualifier`, `decorateTiers`, `validManager`, `scimEnterprise`/`scimManager`. §3.3 migration mark `0059`. |
| 2026-09-16 | Phase 254 (in-portal SSH terminal): `internal/api/terminal_handlers.go` (`sshTerminalToken`, `sshTerminal`, `terminalCredential`, `terminalResize`, `terminalCloseReason`, `Server.SetSSHProxy`, `LoopbackDialAddr`), `Options.{SSHProxyAddr,SSHProxyHostKey}`; `auth.SessionScopeTerminal`, `Principal.{TerminalOnly,TerminalTarget}`, `ScopeTerminal`, `TerminalClientVersion`, `TerminalRemoteFromVersion`; `proxy.isLoopbackAddr`, `admitRequest.serving`, `gateTerminalTarget`, `ext["terminal_remote"]`; `web.XtermJS` + `xtermSrc`/`xtermSrcPlaceholder`; console `loadXterm`/`openTerminal`/`startTerminal`. No migration. |
| 2026-09-16 | Phase 252 (credential-level grants): `store.TargetGrant.CredentialID` + `CoversCredential`, `SubjectGrant.CredentialID`; `auth.canAccess` (the shared body), `CanAccessCredentialAt`, `CanConnectCredentialAt`, `GrantDeadlineFor`, `Reach.CredentialIDs` / `reachCredentials`; `api.authorizedForCredential`, a `credID *int64` on `gateCredentialAccess` / `gateSecretDelivery` / `gateTargetAccess`, `credentialScopeGate`, `agentCanUseCredential`; `grantIn.CredentialID`. §3.3 migration mark `0058`. |
| 2026-09-16 | Phase 250 (target labels + label rules): `internal/store/labels.go` (the label and selector vocabulary — `NormalizeLabels`, `ParseLabels`, `ParseLabelSelector`, `LabelSelector`, `LabelSelectorMatches`), `store.LabelRule`, `Target.Labels`, `TargetGrant.Effect`/`IsDeny`, `SubjectGrant.{Effect,Selector}`, `GrantViaLabel`, `GrantAllow`/`GrantDeny`; `GrantStore.{CreateLabelRule,ListLabelRules,DeleteLabelRule}` in both stores plus pgstore's `labelledTargets`/`labelRulesFor` helpers; `auth.DeniedByLabelRule` and the deny-first, allow-only-gates reading in `CanAccessTargetAt`, `GrantDeadline`, `ReachableTargets` (`ReachViaLabel`); `internal/api/labelrules.go` (`createLabelRule`, `listLabelRules`, `deleteLabelRule`), `targetIn.Labels` + `labelDetail`. §3.3 migration mark `0057`. |
| 2026-09-15 | Phase 246 (safe permission sets): `internal/store/safepermissions.go` (the vocabulary, `Normalize`/`Parse`/`JoinSafePermissions`, `DefaultSafePermissions`, `GrantPermits`); `Permissions` on `SafeMember`, `TargetGrant`, `SubjectGrant`; `auth.Action` + `CanAccessTargetAt` (`CanConnectTargetAt` is its use action), `GrantDeadline` and `ReachableTargets`/`Reach.Permissions` reading permissions; an `auth.Action` argument on `api.authorizedForTarget`, `gateCredentialAccess`, `gateSecretDelivery`, `gateTargetAccess`, `agentCanSeeTarget`, `authorizeAgentCredential`; `api.scopedApprovalTargets`, `mayDecideRequest`, `requireDecider`. §3.3 migration mark `0056`. |
| 2026-09-15 | Phase 244 (per-session MFA): `store.EffectiveSessionMFA` beside `EffectiveApprovalPolicy`, `Target.RequireSessionMFA`, `Safe.RequireSessionMFA`, `Session.TargetID`; `auth.SessionScopeSessionMFA` / `ScopeSessionMFA` with `Principal.{SessionMFATicket,SessionMFATarget,SessionMFAFactor}`, `auth.VerifySecondFactor` (the one code check — login, the ticket mint and the SSH prompt) and `auth.CheckSessionMFA` (the gate, in `internal/auth/sessionmfa.go`); the proxies' gate 12 (`gateSessionMFA` / `gateSessionMFACheck`, `sessionMFARefusal`), the SSH proxy's `promptSessionMFA` / `answerSessionMFA` / `stashPrincipal` and the shared `targetNamed`; `internal/api/sessionmfa_handlers.go` (`mintSessionMFATicket`, the WebAuthn pair, `sessionMFAGate`, `refuseSessionMFA`), `gateSecretDelivery` / `gateTargetAccess`, `issueSessionBound`, `webauthnWriteBack`. §3.3 migration mark `0055`. |
