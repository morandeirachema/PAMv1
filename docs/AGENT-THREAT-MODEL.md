...
| Notice an owner offboarding can never reach (Phase 175) | `owner_known` on both agent listings, and a WARNING inside the campaign item — an owner is free text and the cascade matches it as a username string, so a typo silently orphans an agent |
