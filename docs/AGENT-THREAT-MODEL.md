...
| Bound what one token can do (Phase 209) | `PAM_BROKER_MAX_CALLS_PER_TOKEN` caps the brokered calls spent while presenting a single token, keyed on its `jti`. The rate limit bounds a burst and the daily budget bounds a total; neither bounds the case an operator actually worries about — one token, handed over for one task, quietly doing two hundred things. A fresh token starts a fresh ceiling, so the control retires a credential rather than punishing an agent. **Keyed on `jti`, never on the caller-declared `session:`**: a ceiling whose key the limited party chooses is not a ceiling. Static agent keys carry no token id and are covered by the per-day budget instead. Refusals audit `agent.token_budget_exhausted` |
| Make a stolen delegated token useless (Phase 206) | the delegator passes `cnf_jkt` on the exchange; the issued token carries an RFC 7800 `cnf: {jkt: …}`, and every call presenting it must send an RFC 9449 `DPoP` proof signed by the matching private key, covering this method and URI, this token (`ath`) and one use (`jti`). A bound delegator cannot mint an unbound token, so the constraint survives the next hop. `PAM_BROKER_REQUIRE_POP` makes binding mandatory for SVID-authenticated agents; refusals audit `agent.pop_denied` with a reason behind the same opaque 401 a bad bearer gets. **The delegator names the key**, so this narrows the blast radius of token THEFT — it is not workload attestation, which stays SPIRE's |
...
one token and one `broker_tokens` row — a row that, since Phase 222, also names the
**subject** that parked the call, so the token spends for that agent and no other:
a stranger presenting a leaked token is answered as if it were a bad one, at the
peek and inside the atomic spend, and its attempt spends nothing.
...
- **…and enforced by a reservation, since Phase 219.** A count followed by a
  call is a check-then-act: a burst arriving together all read the same count,
  all passed, and the limit over-ran by the burst's width — twelve calls against
  a budget of two, twelve executed. So the gate now also writes a reservation
  (`agent_call_reservations`) at the instant of its decision, under the store's
  own serialisation, and exactly the allowed number of a burst get through. The
  trail count still runs first and is still what is reported; the reservation
  sits behind it and can only refuse, never admit — so the two never disagree
  in the permissive direction. A reservation is released when the call does no
  work (policy refusal, failure, approver denial, withdrawal, expiry), so the
  contract above still holds; and a **parked call holds its slot from the
  moment it is requested**, which closes a second hole the same test found: the
  approval path never re-checked the budget, so a budget of one admitted any
  number of approvals.
...
| Control | Proven by |
|---|---|
| 2026-08-26 | Phase 206: **a minted delegated token can be bound to a key.** Every control this model describes for delegated tokens answered *who the token speaks for*; none answered *who is holding it*, so a captured token was the sub-agent until expiry. `POST /v1/token` now accepts `cnf_jkt` and stamps an RFC 7800 `cnf`; a bound token is refused unless the call carries an RFC 9449 `DPoP` proof signed by that key, scoped to this request, this token and one use. A bound delegator cannot mint an unbound token, and a confirmation PAMv1 cannot enforce refuses the token rather than reading as unbound. `PAM_BROKER_REQUIRE_POP` makes it mandatory for SVID-authenticated agents. Refusals audit `agent.pop_denied` with a reason and score as `command_blocked`. **The limit stated plainly**: the delegator names the key, so this bounds THEFT of a token, not the binding of a key to a workload — SPIRE still owns that |
| 2026-08-25 | Phase 204: MCP `tools/list` no longer returns the whole registry to every agent. It previously named `winrm_exec` and `reveal_credential` to an agent that could call neither — policy refused the call, but the surface had already been described, which is the disclosure Phase 189 closed for target listings. Filtering is advisory by design: it removes a map, not a boundary, and `tools/call` is unchanged |
...
| A trust-domain key rotation is followed without a restart, and a broken rewrite of the bundle does not become an outage (Phase 224) | `agentid.TestSVIDBundleReloadsOnRotation` (a new key starts verifying and a retired one stops, no restart), `TestSVIDBundleKeepsLastGoodOnBadRewrite`, `TestSVIDBundleReloadPreservesIssuer` (the broker's own key survives, a shadowing bundle is refused whole) |
| A leaked resume token is worth nothing to another agent, and costs its owner nothing (Phase 222) | `broker.TestResumeRejectsForeignCollector` (a stranger sharing the owner's name with a different key id), `api.TestBrokerResumeTokenBoundToCollector` (another agent holding the token and the call id, over REST), and the binding on both backends under `storetest` |
| A burst at the budget's boundary cannot over-run it, and a refused call gives its slot back (Phase 219) | `api.TestBudgetBurstAtTheBoundaryCannotOverrun` (twelve against two: exactly two), `TestRefusedCallGivesItsBudgetSlotBack`, `TestParkedCallHoldsItsBudgetSlot`, `TestApproverDenialGivesTheBudgetSlotBack`; the store primitive under `storetest` on both backends (twenty goroutines at a limit of five: exactly five) |
