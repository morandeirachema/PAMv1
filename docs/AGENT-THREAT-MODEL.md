...
window, and `scope` is documented as the audit label it always was.
>
...
| Know who to hold responsible for an attested agent (Phase 170) | `POST /v1/agents/identities` records the accountable human for a SPIFFE ID (`manage_users`, console menu 26 → **F8**). Not enrollment and not attestation: it admits nothing and proves nothing, it names somebody |
...
**Still open, and not obscured by this**: pamv1 keeps no *inventory* of SVID
identities. Any workload in the trust domain is an agent whether or not anyone
registered it; the registry records owners for the ones somebody named, it does
not enumerate the ones nobody did. First-seen/last-seen tracking and an opt-in
"only enrolled SVIDs may call" mode are the next step, and are not built.
