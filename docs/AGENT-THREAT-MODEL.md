...
| The owner left | deleting the human suspends every agent key they owned, audited `reason:owner-offboarded` |
...
**Still open, and named here rather than implied**: a *minted delegated token*
(Phase 57) has no revocation list, so its TTL remains its containment — see
Known boundaries below. Quarantining the delegator's subject stops it from
minting more and stops its own calls, but a token already in an agent's hands
