...
abarca de la 0 a la 227 y de la 229 a la 267, **se han entregado todas las fases**, y la release etiquetada y
...
Fases 0–227 y 229–265, agrupadas por área. Cada capacidad está cubierta por tests y se despliega como código.

### Identidad y acceso

- **Control de acceso por roles + perfiles personalizados** — cuatro roles integrados (`admin`, `user`, `auditor`, `approver`) más **perfiles de permisos personalizados**: nombra cualquier conjunto de capacidades (`read_inventory`, `manage_credentials`, `connect`, `reveal_secret`, `read_audit`, `approve`, …) y asígnalo a un usuario como un rol. Una única matriz rol/perfil→capacidad aplicada por la API **y** el proxy; los admins emiten tokens por usuario (guardados como SHA-256); cada denegación se audita con el usuario real. Un administrador puede **bloquear** una identidad (motivo, `until` opcional) — su token, sus sesiones y nuevos inicios de sesión se detienen al instante — y **rotar** su token; los tokens pueden caducar (`PAM_USER_TOKEN_TTL_HOURS`, fase 242).
...
- **Doble factor TOTP** — alta autoservicio ([RFC 6238](https://datatracker.ietf.org/doc/html/rfc6238), cualquier app de autenticación); el secreto se guarda cifrado en el vault y el login exige el código de 6 dígitos una vez dado de alta. Códigos de recuperación de un solo uso y una política opcional de MFA obligatoria (con primer inicio solo para alta). Un despliegue, un objetivo o un safe pueden exigir además un **segundo factor nuevo en cada sesión** — un código de un solo uso que pide el proxy SSH, o un ticket de un solo uso ligado a un objetivo para los proxies de base de datos, el visor RDP/VNC, revelar y check-out (`PAM_SESSION_MFA`, fase 244).
- **Safes (contenedores de acceso delegado)** — agrupa objetivos en un **safe** con sus propios miembros; un miembro puede conectar a **todos los objetivos del safe** (una vía de autorización junto a las concesiones por objetivo), y un miembro `can_manage` es un **administrador delegado del safe**. Poner un objetivo en un safe lo restringe a sus miembros. `POST /api/safes`, `/api/safes/{id}/members`, `PUT /api/targets/{id}/safe`. Un grant o una membresía puede llevar una **caducidad** y una **franja horaria** (`"Mon-Fri 08:00-18:00 Europe/Madrid"`, fase 240): fuera de ellas no autoriza, y una sesión admitida bajo ellas termina en su límite. `PAM_SESSION_MAX_MIN` / `PAM_SESSION_IDLE_MIN` acotan la duración y la inactividad de toda sesión. Una membresía nombra lo que concede (fase 246): `use` (conectar), `retrieve` (revelar / check-out) y `approve` (decidir las solicitudes de acceso del safe sin el rol global de aprobador).
...
v0.76.0**. Las releases quedan registradas en **[CHANGELOG.md](CHANGELOG.md)**;
...
| ~~**Sonda de sesión Windows**~~ **✅ entregado (Fase 266)** | Telemetría y bloqueo *dentro* de una sesión RDP intermediada — qué programas ejecuta el operador y a dónde conectan, con reglas de bloqueo para ambos (sensores tipo EDR; el equivalente en escritorio del control de comandos) | El mismo `pam-agent`, `PAM_AGENT_MODE=probe`, lanzado al iniciar sesión dentro de la sesión Windows del operador **con el token de ese usuario**: informa de los procesos y conexiones de la sesión por un canal que pam-server abre hacia él y termina lo que las reglas casan — solo lo que el usuario podría terminar, sin reglas de cortafuegos, auditado por cada actuación, nunca una frontera de contención. Menú 34; `/api/probes*`, `/api/probe-rules*` |
...
**[v0.76.0](https://github.com/morandeirachema/pamv1/releases/tag/v0.76.0)**. La lista completa está en
