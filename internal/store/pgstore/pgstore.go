...
func (s *PGStore) SweepExpiredGrants(ctx context.Context, now time.Time) ([]store.TargetGrant, []store.SafeMember, error) {
	...
	ms, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (store.SafeMember, error) {
		...
		var perms string
		err := row.Scan(&m.ID, &m.SafeID, &m.SubjectType, &m.Subject, &m.CanManage, &m.CreatedBy, &m.ExpiresAt, &m.TimeFrame, &perms)
		m.Permissions = store.ParseSafePermissions(perms)
		return m, err
	})
	if err != nil {
		return nil, nil, err
...
func (s *PGStore) AddSafeMember(ctx context.Context, m *store.SafeMember) error {
	if m.Permissions == nil {
		m.Permissions = store.DefaultSafePermissions()
	}
	err := s.pool.QueryRow(ctx,
		`INSERT INTO safe_members (safe_id, subject_type, subject, can_manage, created_by, expires_at, time_frame, permissions) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`,
		m.SafeID, m.SubjectType, m.Subject, m.CanManage, m.CreatedBy, m.ExpiresAt, m.TimeFrame, store.JoinSafePermissions(m.Permissions),
	).Scan(&m.ID)
	switch pgCode(err) {
	case pgUniqueViolation:
		return store.ErrConflict
...
func (s *PGStore) ListSafeMembers(ctx context.Context, safeID int64) ([]store.SafeMember, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT id, safe_id, subject_type, subject, can_manage, created_by, expires_at, time_frame, permissions FROM safe_members WHERE safe_id = $1 ORDER BY id`, safeID)
	if err != nil {
		return nil, err
	}
