...
func TestValidatorWebhook(t *testing.T) {
	t.Run("2xx accepts", func(t *testing.T) {
		...
		t.Cleanup(srv.Close)
		v, err := New("", NewWebhook(srv.URL, nil))
		if err != nil {
			t.Fatal(err)
		}
		if err := v.Validate(context.Background(), "CHG1234", "alice"); err != nil {
	...
	t.Run("unreachable ITSM denies (fail closed)", func(t *testing.T) {
		...
		v, err := New("", NewWebhook(url, nil))
		if err != nil {
			t.Fatal(err)
		}
		err = v.Validate(context.Background(), "CHG1234", "alice")
		if err == nil {
...
func TestValidatorPatternAndDisabled(t *testing.T) {
	...
	if err := v.Validate(context.Background(), "whatever", "alice"); err != nil {
	...
	pv, err := New(`^CHG[0-9]{3,}$`, nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := pv.Validate(context.Background(), "CHG1234", "alice"); err != nil {
		t.Fatalf("valid ticket rejected: %v", err)
	}
	if err := pv.Validate(context.Background(), "nope", "alice"); err == nil {
